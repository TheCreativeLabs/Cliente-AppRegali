    ...
    public class HideableToolbarItem : ToolbarItem
        {
            ...
            private static void OnIsVisibleChanged(BindableObject bindable, bool oldvalue, bool newvalue)
            {
                ...
                var items = item.Parent as ContentPage;

                if (newvalue && !items.ToolbarItems.Contains(item))
                {
                    items.ToolbarItems.Add(item);
                }
                else if (!newvalue && items.ToolbarItems.Contains(item))
                {
                    items.ToolbarItems.Remove(item);
                }
        }
        }
}
